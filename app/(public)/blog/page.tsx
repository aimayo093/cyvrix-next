...
export default function BlogListPage() {
  return (
    <div className="pt-24 pb-32 bg-slate-50">
      <div className="max-w-7xl mx-auto px-5 lg:px-8">
        ...
        <div className="max-w-3xl mb-20">
          ...
          <h1 className="font-outfit text-5xl md:text-6xl font-black text-[#041635] mt-6 mb-6 leading-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group flex flex-col bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-[#2691F0] transition-all overflow-hidden"
            >
              <div className="p-8 flex flex-col flex-grow">
                <div className="flex items-center gap-2 mb-6">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#2691F0] bg-blue-50/80 px-2.5 py-1 rounded-md border border-[#2691F0]/10">
                    {post.category}
                  </span>
                </div>
                
                <h3 className="font-outfit text-2xl font-black text-[#041635] group-hover:text-[#2691F0] transition-colors leading-tight mb-4">
                  {post.title}
                </h3>
                ...
                <div className="border-t border-slate-100 pt-6 flex items-center justify-between text-xs text-slate-400 font-semibold mt-auto">
